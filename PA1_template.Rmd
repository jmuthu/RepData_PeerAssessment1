...
Mean and Median of the total number of steps taken per day are `r mean_day`
and `r median_day` respectively
...
plot(x = labels(avg_daily)[[1]], y = as.numeric(avg_daily),
     xlab="Time interval (minute)", 
     main="Average number of steps taken for all days", type="l")
```

`r maximum_avg_index` interval contains the maximum average number of steps which is `r as.numeric(avg_daily[maximum_avg_index])   `
...
Mean and Median of the total number of steps taken per day are `r filled_mean_day`
and `r filled_median_day` respectively. Imputting the missing values has no 
difference on the mean because the missing data was filled with means but the 
...
xyplot(value~Interval | DayType, data = melt_filled_activity_avg, 
       type="l", layout = c(1,2), ylab="Number of Steps",scales=list(x=list(0,500,1000,1500,2000,2500)))
